...
const elements = {
    ...
    recommendedSkillsContainer: document.getElementById('recommendedSkillsContainer'),
...
function initializeEventListeners() {
    // File upload
    elements.resumeUploadArea.addEventListener('click', () => elements.resumeInput.click());
...
function validateAndSetFile(file) {
    ...
    if (file.size > CONFIG.MAX_FILE_SIZE) {
        showError(`File size must be less than ${CONFIG.MAX_FILE_SIZE / 1024 / 1024}MB.`);
        return;
    }
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            showError('Unable to connect to the API server. Please ensure the backend is running on ' + CONFIG.API_BASE_URL);
        } else {
...
function displayMatchScore(score) {
    ...
    if (scoreValue >= 80) {
        interpretation = '🎉 Excellent match! Your resume aligns very well with the job requirements.';
        interpretationClass = 'excellent';
    } else if (scoreValue >= 60) {
        interpretation = '👍 Good match! You meet most of the requirements with some areas for improvement.';
        interpretationClass = 'good';
    } else if (scoreValue >= 40) {
        interpretation = '⚠️ Moderate match. Consider enhancing your resume to better align with the job.';
        interpretationClass = 'moderate';
    } else {
        interpretation = '⚡ Low match. Significant improvements needed to meet job requirements.';
...
function displaySuggestions(suggestions) {
    ...
    if (!suggestions || suggestions.length === 0) {
        elements.suggestionsContainer.innerHTML = '<p class="empty-state">No specific suggestions at this time.</p>';
        return;
    }
...
function displayRecommendedSkills(skills) {
    ...
    if (!skills || skills.length === 0) {
        elements.recommendedSkillsContainer.innerHTML = '<p class="empty-state">No additional skills recommended.</p>';
        return;
    }
...
function copyAnalysis() {
    ...
    navigator.clipboard.writeText(currentAnalysisData.raw_analysis)
        .then(() => {
